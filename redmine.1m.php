...
// Sets the colors for the statuses, also used for sorting by status
$statusColor = [
    'Neu'            => 'darkred',
    'In Bearbeitung' => '#E9D66B',
    'Gelöst'         => 'darkgreen',
    'Staging'        => '#000000',
    'Live'           => '#555555',
];

// set the colors for your status when you use darkmode
if (getenv('XBARDarkMode') === 'true') {
    $statusColor = [
        'Neu'            => '#ff7f7f',
...
foreach ($projects as $projectId => $projectName) {
    echo $projectName . ' (' . $ticketCount[$projectId] . ') ' .
        '| href=' . $redmineUrl . 'projects/' . $projectId . PHP_EOL;

    uksort(
        $issues[$projectId],
        static function ($a, $b) use ($statusOrder) {
    ...
    foreach ($issues[$projectId] as $status => $issueList) {
        foreach ($issueList as $issueId => $issue) {
            ...
            echo $statusColor[$status] ? ' color=' . $statusColor[$status] : '';
            echo PHP_EOL;
        }
    }
    echo '---' . PHP_EOL;
